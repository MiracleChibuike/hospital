...
Capital Health (C-Health) is a Hospital Management System aimed at creating a more suitable environments for patients and doctors
...
- Create a Folder on your machine (mkdir `folder_name`)
- Navigate into the folder (cd `folder_name`)
...
- Run the command 
```bash
git clone https://github.com/CapitalhealthCareProject/Capital_Heath_org.git
```
...
- After the creation of your branch, to switch to your new branch, then you can do:
  ...
  ```
We currently have 3 branches, (master, tchidi and frontend-end_Base).

- All our completed and vetted jobs will be residing in the master branch, so before you start working, do the following to see what others have done so you do not repeat what someone else is working on:
...
- Then go back to github and create a pull request to the branch 'master'
